...
// Extractor implements the FastExtractor interface for FB2 files
type Extractor struct{}

// ExtractCoverFromFile extracts only the cover image from an FB2 file
func (e *Extractor) ExtractCoverFromFile(filePath string) ([]byte, string, error) {
	return ExtractCoverOnly(filePath)
}

// ExtractCoverFromReader extracts only the cover image from an FB2 reader
func (e *Extractor) ExtractCoverFromReader(r io.ReaderAt, size int64) ([]byte, string, error) {
	return ExtractCoverOnlyReader(r, size)
}

// ExtractAnnotationFromFile extracts only the annotation from an FB2 file
func (e *Extractor) ExtractAnnotationFromFile(filePath string) (string, error) {
	return ExtractAnnotationOnly(filePath)
}

// ExtractAnnotationFromReader extracts only the annotation from an FB2 reader
...
// ExtractMetadataFromFile extracts only metadata from an FB2 file
func (e *Extractor) ExtractMetadataFromFile(filePath string) (parser.Metadata, error) {
	return ExtractMetadataOnly(filePath)
}

// ExtractMetadataFromReader extracts only metadata from an FB2 reader
